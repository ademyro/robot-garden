...
/**
 * Sets up the common uniform values for most shaders, if it exists.
 *
 * Otherwise, this function does nothing.
 */
static void setupShaderUniforms(const Shader shader) {
  GLint texLoc = glGetUniformLocation(shader.id, "ourTexture");
...
Maybe(Shader) newShader(const char *folderName) {
  ...
  if (shader.isSome) {
    setupShaderUniforms(shader.value);
  }
